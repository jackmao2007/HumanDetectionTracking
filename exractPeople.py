...
def find_detections(img, human_model, pose_model, image_scaling):
    ...
    good_boxes = non_max_supression(people, 0.45) # peope      # = people for not supressed
...
# image name
sample = walkpath + '/pedestrians-walking.jpg'
files = listdir(gdrive + standpath)
...
out = cv2.VideoWriter(gdrive + 'output.avi', fourcc, 25.0, (740, 416), True)
...
while counter < 650:
    ...
    frame = cv2.resize(frame, (740, 416))
    counter += 1
    # detect people every some frames
    if counter % 5 == 0:
